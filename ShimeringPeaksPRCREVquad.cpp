//g++ -Istk/include/ -Lstk/src/ -D__MACOSX_CORE__ ShimmeringPeaksPRCREVquad.cpp -lstk -lpthread -framework CoreAudio -framework CoreMIDI -framework CoreFoundation -o ShimmeringPeaks-PRCRev-Quad
...
PitShift PitchShift2_L;
PitShift PitchShift2_R;
BiQuad hipass;
BiQuad lowpass;
BiQuad Peak_1;
BiQuad Peak_2;
...
//Create registers needed for real time input & output, and the pointer for the Sinewave to generate properly in the callback function
    ...
    for ( unsigned int i=0; i<nBufferFrames; i++ ){
        ...
        *samples++ = sample * OutGain;
        ...
        PitchShift2_L.tick(Verb_Temp2_L);
        PitchShift_Sum_L = 0.5 * (PitchShift2_L.lastOut() + PitchShift_L.lastOut());
        hipass.tick(PitchShift_Sum_L);
        ...
        PitchShift2_R.tick(Verb_Temp2_R);
        PitchShift_Sum_R = 0.5 * (PitchShift_R.lastOut() + PitchShift2_R.lastOut());
        hipass.tick(PitchShift_Sum_R);
        lowpass.tick(hipass.lastOut());
...
int main(int argc, char *argv[])
{
    ...
    if ( argc != 15 ) {
        ...
        std::cout << "ShiftL controls the pitch shift of first pitch shifter on the left channel, where 1. is no change, 0.5 is one octave down and 2. is one octave up" << std::endl;
        std::cout << "usage: " << argv[6] << " Right Shift Factor" << std::endl;
        std::cout << "ShiftR controls the pitch shift of second pitch shifter on the right channel, where 1. is no change, 0.5 is one octave down and 2. is one octave up" << std::endl;
        ...
        std::cout << "Shift2L controls the pitch shift of the second pitch shifter on the left channel, where 1. is no change, 0.5 is one octave down and 2. is one octave up" << std::endl;
        std::cout << "usage: " << argv[14] << " Right Shift Factor 2" << std::endl;
        std::cout << "Shift2R controls the pitch shift of the second pitch shifter on the right channel, where 1. is no change, 0.5 is one octave down and 2. is one octave up" << std::endl;
        return(0);
    }
    ...
    static StkFloat Shift2L = atof(argv[13]);

    if (Shift2L <= 0){
        std::cout << "Second Left Shift Factor should be more than zero.... setting 4." << std::endl;
        Shift2L = 4.;
    }
    static StkFloat Shift2R = atof(argv[14]);

    if (Shift2R <= 0){
        std::cout << "Second Right Shift Factor should be more than zero.... setting to 0.8" << std::endl;
        ShiftR = 0.8;
    }
    ...
    PitchShift2_L.setShift(Shift2L);
    PitchShift2_L.setEffectMix(1);
    PitchShift2_R.setShift(Shift2R);
    PitchShift2_R.setEffectMix(1);
